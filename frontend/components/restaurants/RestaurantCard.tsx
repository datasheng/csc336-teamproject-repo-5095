...
export default function RestaurantCard({ restaurant }: RestaurantCardProps) {
  ...
  // This allows the component to work with both mock data and real API data
  const id = restaurant.id || restaurant.RESTAURANT_ID;
  const name = restaurant.name || restaurant.RESTAURANT_NAME;
  const image = restaurant.image || '/images/placeholder.jpg';
  const description = restaurant.description || `Experience authentic cuisine at ${name}`;
      ...
      <div className="bg-white rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden group cursor-pointer transform hover:-translate-y-2">
        ...
        <div className="relative h-56 overflow-hidden">
          ...
          <div className="absolute bottom-4 right-4 bg-white rounded-2xl px-4 py-2 shadow-lg flex items-center gap-2 group-hover:scale-110 transition-transform duration-300 z-10">
            ...
            <span className="text-sm font-bold text-[#2C3E50]">{rating.toFixed(1)}</span>
          </div>
        </div>
        
        {/* Content */}
        <div className="p-6">
          <h3 className="text-2xl font-bold text-[#2C3E50] mb-2 group-hover:text-[#5B2C91] transition-colors duration-300">
            {name}
          </h3>
          ...
          <div className="flex items-center justify-between">
            <span className="bg-gradient-to-r from-[#5B2C91] to-[#8B6FB0] text-white px-4 py-2 rounded-full text-sm font-bold">
              {cuisine}
            </span>
