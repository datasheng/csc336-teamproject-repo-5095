...
def populate_database():
    ...
    # Step 1: Users already added ✅
    cursor.execute("SELECT COUNT(*) FROM USERS")
    user_count = cursor.fetchone()[0]
    print(f"\n👥 Users: {user_count} found ✅")
    
    # Step 2: Restaurants already added ✅
    ...
    # Step 3: Add menu items with CORRECT column names
    ...
    if menu_count == 0:
        ...
        # Columns: RESTAURANT_ID, ITEM_NAME, ITEM_DESCRIP, PRICE
        menu_items = [
        ...
        for item in menu_items:
            # FIXED: ITEM_DESCRIP and PRICE (not ITEM_DESCRIPTION and ITEM_PRICE)
            cursor.execute("""
    ...
    for i in range(60):
        try:
            ...
            # FIXED: Use PRICE column (not ITEM_PRICE)
            cursor.execute(
            ...
            delivery_fee_total = Decimal(str(random.uniform(3.00, 7.00))).quantize(Decimal('0.01'))
            delivery_platform_cut = (delivery_fee_total * Decimal('0.30')).quantize(Decimal('0.01'))
            ...
            """, (order_id, driver_id, delivery_status, estimated, actual,
                  delivery_fee_total, delivery_platform_cut))
